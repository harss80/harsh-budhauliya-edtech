                        ...
                        {step === 1 && (
                            <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                ...
                                <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    {!isLogin && <div style={{ position: 'relative' }}><User size={18} style={iconLocStyle} /><input type="text" name="name" placeholder="Full Name" value={authData.name} onChange={handleAuthChange} style={{ ...inputStyle, paddingLeft: '48px' }} /></div>}
                                    <div style={{ position: 'relative' }}><User size={18} style={iconLocStyle} /><input type="email" name="email" placeholder="Email Address" value={authData.email} onChange={handleAuthChange} style={{ ...inputStyle, paddingLeft: '48px' }} /></div>
                                    <div style={{ position: 'relative' }}><Lock size={18} style={iconLocStyle} /><input type={showPassword ? "text" : "password"} name="password" placeholder="Password" value={authData.password} onChange={handleAuthChange} style={{ ...inputStyle, paddingLeft: '48px', paddingRight: '48px' }} /><button type="button" onClick={() => setShowPassword(!showPassword)} className="btn-reset" style={passIconStyle}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>
                                    {!isLogin && <div style={{ position: 'relative' }}><Lock size={18} style={iconLocStyle} /><input type="password" name="confirmPassword" placeholder="Confirm Password" value={authData.confirmPassword} onChange={handleAuthChange} style={{ ...inputStyle, paddingLeft: '48px' }} /></div>}
                                    {error && <div style={errorStyle}>{error}</div>}
